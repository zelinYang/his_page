/*--------------------------------------------------------------------------  
 *
 * GXCA Adaptive Javascript, Version SAB(Support All Browsers :))
 * This script support GXCA client version 2.0 and later
 * Author:GXCA-zys
 *--------------------------------------------------------------------------*/
...
function setUserName()
{
	if(isOpenSelfFn){
		if($_$hardCertListID != ""){
			var obj = document.getElementById($_$hardCertListID.split(".")[1]);
			for (i=0;i<obj.length;i++){ 
				if (obj[i].selected== true ) {
					//alert(obj[i].text);
					//document.getElementById("username").value = obj[i].text;
				}	
			}	
		}
	}
...
function $loginGetPINRetryCallBack(retObj)
{
    ...
    if (retryCount > 0) {
        ...
        $XTXAlert("您的证书密码已被锁死,请联系GXCA进行解锁!");
        return;
    } else {
        $XTXAlert("登录失败!");
        return;
