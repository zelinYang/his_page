...
layui.define(["laytpl","layer","form"], function (exports) {
    ...
    var form = layui.form;
    layui.link('../resource/layuiadmin/eleTree/eleTree.css');
            ...
            if(self.option.url){
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        layer.alert(data.data, { title: "选择器"+self.elem+"获取数据失败", icon: 2 });
                    }
                });
                return;
            }
                ...
                var b=['<div class="eleTree-node ',spread?'eleTree-hide':"",'" eleTree-floor="'+count+'">'
                    ,'<div class="eleTree-node-content" style="padding-left: '+18*count+'px;">'
                        ,'<span class="eleTree-node-content-icon">'
                            // 判断叶子节点
                            ,(function() {
                                if((val.children && val.children.length > 0) || val.leaf){
        ...
        rightClickMenu: function() {
            ...
            $(this.elem).on("contextmenu",".eleTree-node-content",function(e) {
                ...
                $("#tree-menu li.add").off().on("click",function() {
                    ...
                    var data=self.reInitData(node);
                    var d=data.currentData;
                    if (self.option.customAdd && typeof(self.option.customAdd) === 'function') {
                        self.option.customAdd({data: self.data, currentData: d});
                        return ;
                    }
                ...
                $("#tree-menu li.edit").off().on("click",function() {
                    ...
                    if (self.option.customEdit && typeof(self.option.customEdit) === 'function') {
                        self.option.customEdit({data: self.data, currentData: d});
                        return ;
                    }
                    layer.prompt({
                ...
                $("#tree-menu li.remove").off().on("click",function() {
                    ...
                    if (self.option.customRemove && typeof(self.option.customRemove) === 'function') {
                        self.option.customRemove({data: self.data, parentData: d,currentData: data.currentData});
                        return ;
                    }
