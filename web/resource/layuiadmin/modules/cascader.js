...
layui.define(["jquery","laytpl","layer"], function (exports) {
    layui.link('../resource/layuiadmin/modules/custom/css/cascader.css');
    var $ = layui.jquery;
    ...
    Cascader.prototype={
            ...
            if(self.option.url){
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        layer.msg("获取数据失败", {icon: 2, time: 1200});
                    }
                });
                return;
            }
        ...
        // 初始化容器和标签
        init: function () {
            $(this.elem).after('<i class="layui-icon layui-icon-down select-input-down"></i>');
        ...
        initFirst: function () {
            ...
            this.domContent.find("ul:first").hide();
        },
        // 获取当前点击的当前面板的数据
        ...
        finishInitData: function (triggerData) {
            ...
            $(this.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-down select-input-down"></i>');
        ...
        callback: function () {
            ...
            $(self.elem).on("click", function () {
                ...
                if (self.onOff) {
                    ...
                    $(self.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-up select-input-down"></i>');

                    self.domContent.css("zIndex",zIndex);
                } else {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideUp(100);
                    $(self.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-down select-input-down"></i>');
                }
            })
            // 点击外层文档隐藏
