...
layui.define(['jquery', 'table'], function (exports){
      "use strict";
      layui.link('../resource/layuiadmin/modules/custom/css/yutons_sug.css');
      ...
      yutons_sug.prototype.render = function(opt) {
            ...
            opt.size = opt.size || 'lg'; //小尺寸的表格
            opt.width = opt.width || '100%';

            //初始化输入框提示容器
            $("#" + opt.id).after("<div id='sugItem' class='yutons_sug_box' style='background-color: #fff;display: none;z-index:9999999;position: absolute; width:" + opt.width + ";'></div>");
      ...
      function getSugTable(opt) {
	    ...
	    //下拉表格初始化table容器
	    let html = '<div class="yutons_sug_to_top" style="margin-left:' + ($("#" + opt.id).width()/ 2) + 'px"></div><table id="yutons_sug_' + opt.obj.getAttribute("id") + '" lay-filter="yutons_sug_' + opt.obj.getAttribute("id") + '"></table>';
      ...
      function getSug(opt) {
            ...
            //sug下拉框异步加载数据并渲染下拉框
            $.post(opt.urlBak, {keyWord: kw}, function (data){
                let html = "";
                layui.each(data.data, (index, item) => {
                    html += "<div class='item' style='padding: 3px 10px;cursor: pointer;' onmouseenter='getFocus(this)' row-data='" + JSON.stringify(item) + "' name='yutons_sug_call'>" + item[opt.labelName] + "</div>";
                });
                if (html != "") {
                    $("#" + sessionStorage.getItem("inputId")).next().show().html(html);
                    $('div[name="yutons_sug_call"]').click(function (){
                        var data = JSON.parse($(this).attr('row-data'));
                        opt.callBack(data);
                    });
                } else {
      ...
      let movePrev = function(id) {
            ...
            if (index == 0) {
                  $(".item").removeClass('addbg').css("background", "").eq($(".item").length - 1).addClass('addbg').css("background", "#e6e6e6");
            } else {
